    ...
    for epoch_num in range(num_epochs):
        ...
        while True:
            try:
                ...
                # valX, valY, img_data = next(data_gen_val)
                # valid_loss_rpn = model_rpn.test_on_batch(valX, valY)
                ...
                if iter_num == epoch_length:
                    ...
                    '''
                    valX, valY, val_img_data = next(data_gen_val)
                    val_loss_rpn = model_rpn.train_on_batch(valX, valY)

                    valP_rpn = model_rpn.predict(valX)
                    val_result = roi_helpers.rpn_to_roi(valP_rpn[0], valP_rpn[1], cfg, K.image_dim_ordering(), use_regr=True,
                                                    overlap_thresh=0.7,
                                                    max_boxes=300)

                    valX2, valY1, valY2, valIouS = roi_helpers.calc_iou(val_result, val_img_data, cfg, class_mapping)
                    '''
